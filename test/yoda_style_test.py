#!/usr/bin/env python3
"""
Live test: Yoda persona vs neutral baseline.

Verifies that the Yoda overlay produces distinctly Yoda-like phrasing and
structure — fronted emphasis, aphoristic compression, long-term consequence
framing, balance/burden vocabulary — while preserving task correctness.

Supports both Anthropic (Claude) and OpenAI APIs.

Usage:
    ANTHROPIC_API_KEY=... python3 yoda_style_test.py
    OPENAI_API_KEY=... OPENAI_MODEL=gpt-4o python3 yoda_style_test.py --api openai
    python3 yoda_style_test.py --case technical-debt --show-outputs --dry-run
"""
...
from packet_builder import load_neutral_packet, load_packet

TEST_DIR = Path(__file__).resolve().parent
CASES_PATH = TEST_DIR / "yoda_cases.json"
...
def build_judge_prompt(case: dict[str, object], neutral_output: str, yoda_output: str) -> str:
    return textwrap.dedent(
        f"""
        Evaluate two assistant responses to the same task. One is a neutral baseline.
        The other applies a Yoda personality overlay. Judge on two axes:
        (1) task correctness and usefulness, (2) how distinctly the Yoda persona
        shapes the phrasing, structure, and framing.

        Yoda markers to look for:
        - fronted emphasis: the key idea placed before the verb or subject
          (e.g. "Dangerous, this path is." not "This path is dangerous.")
        - short aphoristic verdict lines that compress the lesson
        - Yoda vocabulary: burden, haste, discipline, balance, clouded, path,
          simpler way, matters not, much to learn, fear, what endures
        - long-term consequence framing: naming what burden the current choice creates
        - teacher stance: probing beneath the surface question before recommending
        - occasional wry needling humor
        - NOT required: full inversion in every sentence — variety is correct

        The voice should be recognizably Yoda without being a caricature.
        At least a few unmistakably Yoda-like phrasings must be present.
        ...
        {{
          ...
          "yoda_shows_persona": true,
          "yoda_uses_fronted_emphasis_or_inversion": true,
          "yoda_uses_aphoristic_compression": true,
          "yoda_frames_long_term_consequences": true,
          "difference_is_unmistakable": true,
          "notes": "short explanation of which Yoda markers were present or missing",
          "scores": {{
            "neutral_task": 1,
            "yoda_task": 1,
            "persona_strength": 1,
        ...
        persona_strength and style_difference must be >= 4 to pass.
...
def run_case(*, case, model, judge_model, call, neutral_packet, yoda_packet) -> dict[str, object]:
    neutral_output = call(model=model, system_prompt=neutral_packet, user_prompt=str(case["user_input"]))
    yoda_output = call(model=model, system_prompt=yoda_packet, user_prompt=str(case["user_input"]))
    judge_prompt = build_judge_prompt(case, neutral_output, yoda_output)
    judge_raw = call(
        model=judge_model,
...
def validate_verdict(case_id: str, verdict: dict[str, object]) -> list[str]:
    ...
    for key in [
        ...
        "yoda_shows_persona",
        "yoda_uses_fronted_emphasis_or_inversion",
        "yoda_uses_aphoristic_compression",
        "yoda_frames_long_term_consequences",
    ...
    for key, threshold in [("neutral_task", 3), ("yoda_task", 3), ("persona_strength", 4), ("style_difference", 4)]:
...
def summarize_trials(case_id: str, verdicts: list[dict[str, object]]) -> list[str]:
    trial_failures, passes = [], 0
    for i, v in enumerate(verdicts, 1):
        f = validate_verdict(f"{case_id} trial {i}", v)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Live Yoda persona style test (Claude or OpenAI).")
...
def main() -> int:
    ...
    yoda_packet = load_yoda_packet()

    print("=== yoda persona style test ===")
    ...
    if args.dry_run:
        ...
        print(f"yoda packet:    {len(yoda_packet)} chars")
    ...
    for case in cases:
        ...
        for trial in range(1, args.repeats + 1):
            ...
            try:
                verdict = run_case(
                    case=case, model=model, judge_model=judge_model, call=call,
                    neutral_packet=neutral_packet, yoda_packet=yoda_packet,
                )
            ...
            summary = {k: v for k, v in verdict.items() if k not in {"neutral_output", "yoda_output"}}
