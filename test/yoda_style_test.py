...
Verifies that the Yoda overlay produces visibly different framing — aphoristic
compression, fronted emphasis, wisdom-first reframing — while preserving task
correctness.
...
Usage:
    ...
    # Flags
    python3 yoda_style_test.py --case technical-debt --show-outputs --repeats 3
"""
...
from packet_builder import load_neutral_packet, load_yoda_packet
...
def build_judge_prompt(
    ...
    return textwrap.dedent(
        ...
        (1) task correctness and usefulness, (2) how visibly Yoda's style shapes the
        response structure, framing, and language.

        Yoda markers to look for:
        - fronted emphasis: key insight or conclusion comes first, often in an inverted
          or aphoristic form before the explanation follows
        - aphoristic compression: short, gravity-laden lines that carry weight beyond
          their length — "Haste, the enemy of clarity it is" rather than "moving fast
          causes problems"
        - reframing: Yoda diagnoses a deeper or different problem than the surface
          question before engaging with it directly
        - characteristic vocabulary: burden, haste, discipline, balance, path, clouded,
          matters not, patient, long have I watched, do or do not
        - wisdom-first structure: the lesson or principle is identified before the
          practical recommendation
        - speech inversion or unusual syntax at least occasionally — not every sentence,
          but enough to be recognizable

        The difference should be structural and linguistic, not just tonal. A Yoda
        response that answers the same way as the neutral but adds "Hmm, yes" at the
        start does not pass.
        ...
        {{
          ...
          "yoda_reframes_or_wisdom_first": true,
        ...
        yoda_style and style_difference must be >= 3 to pass.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Live Yoda persona style test (Claude or OpenAI)."
    )
