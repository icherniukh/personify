...
def main() -> None:
    ...
    persona_injection = (root_dir / "docs" / "persona-injection.md").read_text(encoding="utf-8")
    persona_injection_research = (root_dir / "docs" / "persona-injection-research.md").read_text(encoding="utf-8")
    ...
    assert_contains(persona_start, "Activation Lifecycle")
    assert_contains(persona_start, "Activation is not storage")
    assert_contains(persona_start, "Reload the resolved pack before answering if the host context no longer contains the active-mode state")
    ...
    assert_contains(persona_apply, "Activation Lifecycle")
    assert_contains(persona_apply, "Activation is not storage")
    ...
    assert_contains(readme, "docs/persona-injection.md")
    print("ok")

    print("8. canonical activation docs explain mechanics and improvement path")
    assert_contains(persona_injection, "## Activation Lifecycle")
    assert_contains(persona_injection, "## Scope Semantics")
    assert_contains(persona_injection, "## Host Matrix")
    assert_contains(persona_injection, "## Difference From A System Prompt Prepend")
    assert_contains(persona_injection, "## Debugging Activation")
    assert_contains(persona_injection, "Activation is not storage")
    assert_contains(persona_injection, "The host conversation state is the storage mechanism")
    assert_contains(persona_injection_research, "## Decision")
    assert_contains(persona_injection_research, "Keep the deterministic Persona Activation Packet as the default activation model")
    assert_contains(persona_injection_research, "Plain Prepend Baseline")
    assert_contains(persona_injection_research, "PAP + Self-Check")
    print("ok")

    print("9. personality packs stay reusable instead of leaking task-specific review language")
