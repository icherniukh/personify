...
def main() -> None:
    ...
    assert_contains(persona_start, "If the user names a promptonality variant skill, activate that variant directly")
    assert_contains(persona_start, "If the user names a workflow and a personality, compose them explicitly")
    assert_contains(persona_start, "read the matching pack file from `references/personality-packs/` before activating the mode")
    assert_contains(persona_start, "does not automatically create a new installed variant skill")
    print("ok")
    ...
    assert_contains(persona_apply, "If the requested combo already exists as a named variant, use that variant")
    ...
    assert_contains(persona_list, "Listing a personality pack does not mean every possible workflow-plus-pack combination already exists as a named variant skill")
